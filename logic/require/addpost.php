...
if (isset($_POST["ap"])) do {
	...
	if (isset($_POST["name"])) {
		$mysql->query("INSERT INTO `posts` (`anon_author`, `anon_ip`, `location`, `sublocation`, `content`) VALUES (". $mysql->quote(santise($_POST["name"])) .", '". $_SERVER['REMOTE_ADDR'] ."', ". $mysql->quote($_GET["p"]) .", NULL, ". $mysql->quote(santise($_POST["post"])) .");");
	} else {
		$mysql->query("INSERT INTO `posts` (`author`, `location`, `sublocation`, `content`) VALUES (". $_SESSION["id"] .", ". $mysql->quote($_GET["p"]) .", NULL, ". $mysql->quote(santise($_POST["post"])) .");");
	}
