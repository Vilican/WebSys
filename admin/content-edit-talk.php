...
$page["title"] = 'Správa obsahu - editace diskuze';
	...
	if ($pg->num_rows > 0) {
		...
		if ($_SESSION["access_admin_content_edit_all"] > 0 or ($_SESSION["access_admin_content_edit"] > 0 and $pg["author"] == $_SESSION["id"])) {
			...
			if (isset($_POST["submit"])) {
				...
				} elseif (strlen($_POST["access"]) == 0 or strlen($_POST["access2"]) == 0) {
				...
				} elseif (!is_numeric($_POST["access"]) or $_POST["access"] < 0 or !is_numeric($_POST["access2"]) or $_POST["access2"] < 0) {
				...
				} elseif (strlen($_POST["access"]) > 4 or strlen($_POST["access2"]) > 4) {
...
<tr><td>Přístup četní:</td><td><input type="text" name="access" class="form-control" value="'. restore_value($pg["access"], $_POST["access"]) .'"></td></tr>
<tr><td>Přístup psaní:</td><td><input type="text" name="access2" class="form-control" value="'. restore_value($pg["param1"], $_POST["access2"]) .'"></td></tr>
