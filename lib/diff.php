...
# https://github.com/rashid2538/php-htmldiff
	...
	class HtmlDiff {
		...
		private function ProcessReplaceOperation( $operation ) {
			$this->ProcessDeleteOperation( $operation, "diffmod strikethrough" );
		...
		private function ProcessInsertOperation( $operation, $cssClass ) {
			...
			$this->InsertTag( "span", $cssClass, $text );
		}
					...
					$workTag = $this->ExtractConsecutiveWords( $words, 'tag' );
			                if( isset($workTag[0]) && $this->IsOpeningTag( $workTag[ 0 ] ) && !$this->IsClosingTag( $workTag[ 0 ] ) ) {
			                    if( strpos( $workTag[ 0 ], 'class=' ) ) {
			                        $workTag[ 0 ] = str_replace( 'class="', 'class=" ', $workTag[ 0 ] );
			                        $workTag[ 0 ] = str_replace( "class='", 'class=" ', $workTag[ 0 ] );
			                    } else {
			                        $workTag[ 0 ] = str_replace( ">", ' class="">', $workTag[ 0 ] );
			                    }
			                }
